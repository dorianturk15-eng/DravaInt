    ...
    <div className="modal-content settings-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <div className="modal-body settings-body">
        ...
        {tab === 'general' && <div className="settings-section-grid">
          ...
          {matches('kiosk touch shop floor kiosk dodir') && <section className="setting-card"><div><strong>{lang === 'hr' ? 'Kiosk način' : 'Kiosk mode'}</strong><small>{lang === 'hr' ? 'Veći gumbi za dodirne terminale' : 'Larger touch targets for shop terminals'}</small></div><label className="switch"><input type="checkbox" checked={settings.kioskMode} onChange={(event) => updateSettings({ kioskMode: event.target.checked })} /><span /></label></section>}
        ...
        {tab === 'alerts' && <div className="settings-section-grid">
          ...
          {matches('delay alerts mute kašnjenje upozorenja') && <section className="setting-card"><div><strong>{lang === 'hr' ? 'Upozorenja kašnjenja' : 'Delay alerts'}</strong><small>{lang === 'hr' ? 'Probijeni rokovi naloga' : 'Overdue work orders'}</small></div><label className="switch"><input type="checkbox" checked={settings.delayAlertsEnabled} onChange={(event) => updateSettings({ delayAlertsEnabled: event.target.checked })} /><span /></label></section>}
          {matches('capacity alerts mute kapacitet upozorenja') && <section className="setting-card"><div><strong>{lang === 'hr' ? 'Upozorenja kapaciteta' : 'Capacity alerts'}</strong><small>{lang === 'hr' ? 'Preopterećeni strojevi' : 'Overloaded machines'}</small></div><label className="switch"><input type="checkbox" checked={settings.capacityAlertsEnabled} onChange={(event) => updateSettings({ capacityAlertsEnabled: event.target.checked })} /><span /></label></section>}
