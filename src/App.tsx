...
function App() {
  ...
  if (!online) alerts.push({ id: 'connection-offline', severity: 'critical', title: t.appShell.workstationOffline, detail: t.appShell.changesStoredLocallyWill });
  if (pendingChanges > 0) alerts.push({ id: `queue-${pendingChanges}`, severity: 'info', title: t.appShell.changesWaitingSync, detail: lang === 'hr' ? `${pendingChanges} lokalnih promjena nalazi se u sigurnom redu čekanja.` : `${pendingChanges} local changes are safely queued.` });
  if (delayedJobs.length) alerts.push({ id: `delayed-${delayedJobs.map((job) => job.id).join('-')}`, severity: 'critical', title: lang === 'hr' ? `${delayedJobs.length} naloga zahtijeva pažnju` : `${delayedJobs.length} work orders need attention`, detail: t.appShell.dueTimeHasPassed, action: 'progress' });
  if (materialRisks.length) alerts.push({ id: `materials-${materialRisks.map((job) => job.id).join('-')}`, severity: 'warning', title: t.appShell.materialAvailabilityRisk, detail: lang === 'hr' ? `${materialRisks.length} naloga čeka ili kasni s materijalom.` : `${materialRisks.length} work orders have waiting or delayed material.`, action: 'gantt' });
  if (machineConflicts) alerts.push({ id: 'machine-overlap', severity: 'warning', title: t.appShell.machineScheduleOverlap, detail: t.appShell.atLeastTwoOrders, action: 'machines', focusJobId: overlapJob?.id });
  if (overloadedMachines.length) alerts.push({ id: `capacity-${settings.capacityAlertPercent}-${overloadedMachines.map(([machine]) => machine).join('-')}`, severity: 'warning', title: t.appShell.capacityThresholdReached, detail: lang === 'hr' ? `${overloadedMachines.length} ${overloadedMachines.length === 1 ? 'stroj prelazi' : 'stroja prelaze'} ${settings.capacityAlertPercent}% tjednog kapaciteta.` : `${overloadedMachines.length} ${overloadedMachines.length === 1 ? 'machine exceeds' : 'machines exceed'} ${settings.capacityAlertPercent}% weekly capacity.`, action: 'machines' });
  if (settings.absenceAlertsEnabled && absentWorkerIds.size) alerts.push({ id: `absence-${[...absentWorkerIds].join('-')}`, severity: 'info', title: t.appShell.todaySAbsence, detail: lang === 'hr' ? `${absentWorkerIds.size} operatera nije dostupno za raspored.` : `${absentWorkerIds.size} operators are unavailable for scheduling.`, action: 'shifts' });
  if (!alerts.length) alerts.push({ id: `healthy-${today}`, severity: 'success', title: t.appShell.operationsStable, detail: t.appShell.noActiveDelaysConflicts });

  const navigationGroup = t.appShell.navigation;
  const actionGroup = t.appShell.quickActions;
  const commands: CommandItem[] = [
    ...tabs.map((item) => ({ id: `nav-${item.key}`, label: item.label, description: t.appShell.openModule, group: navigationGroup, icon: item.icon, keywords: item.key, run: () => navigate(item.key) })),
    ...(canAccess(role, 'workOrders') ? [{ id: 'new-work-order', label: t.appShell.newWorkOrder, description: t.appShell.startOrderCreationPrinting, group: actionGroup, icon: <IconFlow />, keywords: 'create order print', run: () => navigate('workOrders') }] : []),
    ...(canAccess(role, 'shifts') ? [{ id: 'plan-shifts', label: t.appShell.planShifts, description: t.appShell.generatePublishSchedule, group: actionGroup, icon: <IconCalendar />, keywords: 'schedule workers', run: () => navigate('shifts') }] : []),
    { id: 'open-settings', label: t.appShell.workstationSettings, description: t.appShell.appearanceSecurityPlanning, group: actionGroup, icon: <IconGear />, shortcut: 'Ctrl Alt S', keywords: 'theme compact lock', run: () => setIsSettingsOpen(true) },
    { id: 'lock-now', label: t.appShell.lockNow, description: t.appShell.secureCurrentWorkstation, group: actionGroup, icon: <IconLock />, keywords: 'security pin', run: () => setIsLocked(true) },
  ];

  return <div className={`app-shell${isDimmed ? ' is-dimmed' : ''}`}>
    {secondsRemaining !== null && secondsRemaining <= settings.warningSeconds && <div className="lock-countdown-banner" role="status"><IconLock /><span>{lang === 'hr' ? `Zaključavanje za ${secondsRemaining} s` : `Locking in ${secondsRemaining}s`}</span><button className="btn btn-blue" onClick={resetTimer}>{t.appShell.keepConnected}</button></div>}
    {forbidden && <div className="access-denied-toast" role="alert"><strong>403</strong><span>{t.appShell.yourRoleCannotAccess}</span></div>}
    ...
    <aside className={`side-drawer${drawerOpen ? ' open' : ''}`} aria-hidden={!drawerOpen}>
      ...
      <nav className="drawer-menu" aria-label={t.appShell.mainNavigation}>{tabs.map((item) => <button key={item.key} className={`drawer-menu-btn${tab === item.key ? ' active' : ''}`} onClick={() => { navigate(item.key); setIsSidebarOpen(false); }}><span>{item.icon}</span>{item.label}</button>)}</nav>
    </aside>

    <nav className="top-nav" aria-label={t.appShell.mainNavigation}>
      <div className="nav-brand-group"><button className="nav-icon-button nav-drawer-toggle" onClick={() => setIsSidebarOpen(true)} aria-label={t.appShell.openMenu}><IconMenu /></button><div><div className="brand">{t.appTitle}</div><small className="nav-active-module">{tabs.find((item) => item.key === tab)?.label}</small></div></div>
      ...
      <div className="nav-actions">
        <button className="nav-command-button" onClick={() => setIsCommandOpen(true)} title={t.appShell.commandPaletteCtrlK}><IconSearch /><span>{t.appShell.search}</span><kbd>⌘K</kbd></button>
        <span className={`connection-pill ${online ? 'online' : 'offline'}`} title={online ? (t.appShell.online) : (t.appShell.offline)}><i />{pendingChanges > 0 && <b>{pendingChanges}</b>}</span>
        ...
        <div className="lang-switch"><button className={`lang-btn${t.appShell.text}`} onClick={() => setLang('hr')}>HR</button><button className={`lang-btn${lang === 'en' ? ' active' : ''}`} onClick={() => setLang('en')}>EN</button></div>
