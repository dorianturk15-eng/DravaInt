...
create table if not exists public.workers (
  ...
  -- Pins the worker to one shift permanently; they sit out the weekly rotation. Null = rotates.
  -- Added by migration 0010. The FK is attached after shift_definitions exists (see below) —
  -- that table is declared further down this file, so it cannot be referenced inline here.
  fixed_shift_definition_id bigint,
...
-- Deferred FK for workers.fixed_shift_definition_id (declared above, before this table existed).
do $$ begin
  alter table public.workers
    add constraint workers_fixed_shift_definition_id_fkey
    foreign key (fixed_shift_definition_id) references public.shift_definitions(id) on delete set null;
