...
### 6. ✅ PARTIAL — Offline queue replay bypasses optimistic locking and can duplicate rows
**Fixed:** replayed job updates now carry the expected `version` in the match (optimistic locking on replay — a reconnecting terminal no longer clobbers newer edits); permanently-failing items (version conflict / DB rejection) are **parked to a dead-letter store and skipped** instead of head-of-line blocking the whole queue forever; transient network errors still stop-and-retry preserving order. Parked conflicts are surfaced in the SyncDiagnostics panel with a Clear action. New unit test covers the transient-vs-permanent classifier. **Still open:** insert idempotency (a request that reached the server before the socket dropped can still be inserted twice) — needs a client-generated idempotency key or a unique constraint (e.g. on `job_order`), a schema change tracked alongside item 13.
...
**Progress:** (c) ✅ **done** — a shared `splitMachineChain()` in `cpm.ts` now normalises on `'→'` (whitespace-tolerant) and is used by `cpm.ts`, `capacity.ts`, and `GanttChart.tsx`, so a chain typed without spaces no longer double-counts; covered by a unit test. (a) and (b) — the larger identity/`job_operations` refactor — remain open (they also gate the server-side half of #10 and insert idempotency in #6).
